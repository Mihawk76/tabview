...
while ( $Element < $Max_Element )
	...
	*/
    ...
    while ( $Element < $Max_Element )
    {
 	echo $Name_Node[$Element] . " " . $Node_Name[$Element] . "<br>";
	$Element++;
    }
    // Create connection
    $conn = new mysqli($servername, $username, $password, $dbname);
...
while ( $Element < $Max_Element )
    ...
    while ($Element < $Max_Element)
    {
	$sql = "UPDATE CC1120_TH SET Node_Availability='$Node_Availability_Temp[$Element]' WHERE Node_Name='$Node_Name_Temp[$Element]'";
	if ($conn->query($sql) === TRUE) {
	    //echo "Record updated successfully";
	} else {
    	echo "Error updating record: " . $conn->error;
...
while ( $Element < $Max_Element )
{
	...
	echo "<input type='hidden' name='" . $Name_Node[$Element] . "' value='" . $Node_Name[$Element] . "'>";
